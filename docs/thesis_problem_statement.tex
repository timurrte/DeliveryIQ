...
\begin{enumerate}[label=\textbf{О\arabic*.}, leftmargin=3em]
    ...
    \]
    де $z_{iw} = \displaystyle\sum_{\substack{j \in N \\ j \neq i}} x_{ijw} \in \{0, 1\}$ --- індикатор того, що вузол $i$ обслуговується засобом $w$.
...
\begin{remark}
Обмеження \textbf{О5} одночасно виконує дві функції: (1)~елімінація підциклів за схемою Міллера--Такера--Землін (MTZ), оскільки змінні $\tau_i^{(w)}$ монотонно зростають вздовж маршруту; (2)~контроль часових вікон роботи хабів.
\end{remark}
