...
\textbf{Час проходження} $j$-го ребра $e_j^{(k)} = (v_{j-1}^{(k)}, v_j^{(k)})$ маршруту $P_k$ визначається ефективною швидкістю, що залежить від типу призначеного транспортного засобу та допустимості ребра:
\[
    \bar{u}_j^{(k)} =
    \begin{cases}
        \bar{u}_{\theta_{\xi(k)}}, & \text{якщо } \theta_{\xi(k)} \in M_{e_j^{(k)}}, \\[4pt]
        \bar{u}_{\mathrm{Walk}}, & \text{інакше (Сегмент "остання миля").}
    \end{cases}
\]
Тоді час проходження ребра:
\[
    t_j^{(k)} = \frac{\ell_{e_j^{(k)}}}{\bar{u}_j^{(k)}}.
\]
Основний маршрут виконується одним типом транспортного засобу $\theta_{\xi(k)}$. Якщо пункт призначення $d_k$ є недосяжним для $\theta_{\xi(k)}$ (перебуває поза межами доступної підмережі $E_{\theta_{\xi(k)}}$), кінцевий відрізок маршруту долається пішки зі швидкістю $\bar{u}_{\mathrm{Walk}}$.
...
\begin{enumerate}[label=\textbf{О\arabic*.}, leftmargin=3em]
    ...
    \item \textbf{(Доступність транспорту):} $\forall k \in \mathcal{K}$, $\forall v \in P_k$: $\mathcal{F}_{\theta_{\xi(k)}}(v) = 1 \;\lor\; \mathcal{F}_{\mathrm{Walk}}(v) = 1$.
    \item \textbf{(Допустимість типу транспорту з урахуванням "останньої милі"):} $\forall k \in \mathcal{K}$, $\forall e_{ij} \in P_k$: $\theta_{\xi(k)} \in M_{ij} \;\lor\; \mathrm{Walk} \in M_{ij}$.
