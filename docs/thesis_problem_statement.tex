...
Для хромосоми $\mathbf{c}_w$ визначимо \textbf{вартість маршруту}:
\[
    C(\mathbf{c}_w) = \sum_{l=0}^{n_w} t_{\pi(l),\, \pi(l+1)}^{(w)},
\]
де $\pi(0) = \pi(n_w + 1) = 0$ (депо).

\textbf{Функція пристосованості} визначається як обернена величина вартості:
\[
    f(\mathbf{c}_w) = \frac{1}{C(\mathbf{c}_w) + \varepsilon},
\]
де $\varepsilon > 0$ --- мала стабілізуюча константа ($\varepsilon = 1$ у реалізації). Такий вибір забезпечує: (1)~монотонне зростання $f$ при зменшенні $C$; (2)~природне штрафування нездійсненних маршрутів, оскільки $t_{ij}^{(w)} = \mathrm{PENALTY} = 10^9$ для недосяжних пар --- відповідні хромосоми отримують близьку до нуля пристосованість.
...
\subsubsection{Селекція (елітизм)}

На кожному поколінні $g$ особини популяції $\mathcal{P}^{(g)}$ сортуються за спаданням пристосованості. Верхні $E = \max\bigl(2,\; \lfloor P / 10 \rfloor\bigr)$ особин утворюють \textbf{елітну підмножину} $\mathcal{E}^{(g)}$ і переносяться в наступне покоління без змін:
\[
    \mathcal{E}^{(g)} = \bigl\{ \mathbf{c} \in \mathcal{P}^{(g)} \;\big|\; \mathrm{rank}_f(\mathbf{c}) \leq E \bigr\}.
\]
Батьки для решти нащадків обираються \textbf{рівноймовірно} з $\mathcal{E}^{(g)}$.

\subsubsection{Впорядкований кросовер (Order Crossover --- OX)}

Для двох батьків $\mathbf{p}_1, \mathbf{p}_2 \in \mathcal{E}^{(g)}$ нащадок $\mathbf{h}$ будується так:
...
\begin{enumerate}[leftmargin=2em]
    ...
    \]
    \item Решту позицій заповнити елементами з $\mathbf{p}_2$ у тому порядку, в якому вони з'являються в $\mathbf{p}_2$, пропускаючи ті, що вже присутні в сегменті.
\end{enumerate}

Оператор OX гарантує, що результат $\mathbf{h}$ є коректною перестановкою (кожен пункт доставки зустрічається рівно один раз).
...
\begin{enumerate}[leftmargin=2em]
    ...
    \item \textbf{Селекція:} сортувати за спаданням $f$; виділити елітну підмножину $\mathcal{E}^{(g-1)}$.
    \item \textbf{Відтворення:} $\mathcal{P}^{(g)} \leftarrow \mathcal{E}^{(g-1)}$. Допоки $|\mathcal{P}^{(g)}| < P$:
...
\begin{remark}
Генетичний алгоритм є метаевристикою і не гарантує знаходження глобального оптимуму. Проте він демонструє стійку збіжність до квазіоптимальних розв'язків і масштабується суттєво краще за точні методи (зокрема, повний перебір зі складністю $O(n!)$).
\end{remark}
